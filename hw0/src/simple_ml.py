...
def softmax_loss(Z, y):
    ...
    print(Z[np.arange(Z.shape[0]), y].shape)
    logits = np.log(sum_z) - Z[np.arange(Z.shape[0]), y]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples // batch):
        ...
        I[np.arange(m), batch_y] = 1

        # I = np.onehot(batch_y, num_classes=10)
        exp_Z = np.exp(batch_X @ theta)
        Z = exp_Z / np.sum(exp_Z, axis=1, keepdims=True)
        theta_grad = 1/m * batch_X.T @ (Z - I)
        theta -= lr * theta_grad
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples // batch):
        ...
        m = batch_X.shape[0]
        Z_1 = relu(batch_X @ W1)
        I = np.zeros((m, W2.shape[1]))
        I[np.arange(m), batch_y] = 1
        output = Z_1 @ W2
        G_2 = (softmax(Z_1 @ W2) - I)
        mask = np.where(Z_1 > 0, 1, 0)
        G_1 = mask * (G_2 @ W2.T)
        W_1_grad = 1/m * (batch_X.T @ G_1)
        W_2_grad = 1/m * (Z_1.T @ G_2)
        W1 -= lr * W_1_grad
        W2 -= lr * W_2_grad
    return 
    ### END YOUR CODE
