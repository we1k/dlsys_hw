...
def print_node(node):
    print('node.inputs:', len(node.inputs))
    print('node.op', node.op)
    print('node.data', node.data.shape)
    print('node.grad', node.grad.cached_data.shape)
    ...
    for node in reverse_topo_order:
        ...
        for prev, grad in zip(node.inputs, partial_grad):
            if prev in node_to_output_grads_list:
                node_to_output_grads_list[prev].append(grad)
            else:
                node_to_output_grads_list[prev] = [grad]
    ### END YOUR SOLUTION
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    vis = {}
    ret = []
    topo_sort_dfs(node_list[0], vis, ret)
    return ret
    ###
    ##  bfs search
    ##  calculate in_degree
    # in_degree = {}
    # dfs_helper(node_list[0], in_degree)
    # queue = []
    # ret = []
    # # put zero_degree in queue
...
def dfs_helper(node, in_degree):
    if node.inputs == None:
        return
    for prev_node in node.inputs:
        dfs_helper(prev_node, in_degree)
        if prev_node not in in_degree:
            in_degree[prev_node] = 1
        else:
            in_degree[prev_node] += 1
    return
